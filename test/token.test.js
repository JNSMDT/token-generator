...
import { generatePassword, generateToken } from '$lib/functions/token.js';
...
describe.concurrent('Password generator function', () => {
	...
	test.each(['*+', ',-_.', '()/@!?', '#$%&"\':;', '[]^`{}~<=>'])('whitelist the characters "%s"', (whitelist) => {
	...
	test.each(['*+', ',-_.', '()/@!?', '#$%&"\':;', '[]^`{}~<=>'])('blacklist the characters "%s"', (blacklist) => {
		const password = generatePassword(100, {
